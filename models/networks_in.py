    ...
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, padding_type='reflect'):
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.Upsample(scale_factor=2, mode='bilinear'),
                      nn.Conv2d(ngf * mult, int(ngf * mult / 2), kernel_size=3, padding=1, stride=1, bias=use_bias),
...
def _make_nConv(nchan, depth, norm_layer = get_norm_layer(norm_type='instance')):
    ...
    if depth >=0:
        for _ in range(depth):
            layers.append(norm_layer(nchan))
...
class UpConcat(nn.Module):
    def __init__(self, inChans, hidChans, outChans, nConvs, dropout=False,stride=2, norm_layer = get_norm_layer(norm_type='instance')):
        ...
        self.bn1 = norm_layer(hidChans)
        self.do1 = passthrough
        self.do2 = nn.Dropout2d() # Commented out in 3D version
