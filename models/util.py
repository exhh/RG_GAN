...
def mean_squared_error(y_true, y_pred, y_mask, annotate_mask=None):
    ...
    if annotate_mask is not None:
        ...
        for i in range(nN):
            if hasattr(y_pred, 'requires_grad'): # training stage
                if torch.sum(annotate_mask[i]).data.cpu().numpy() > 0:
                    ...
                    masked[i] = naive_loss[i] * y_mask[i] * annotate_mask[i] # this will be 0
...
def dice(y_true, y_pred, annotate_mask=None, smooth=1, epsilon=1e-6, weight=None):
    if annotate_mask is not None:
        ...
        b_pred_masked = b_pred_n * annotate_mask
        assert b_pred_masked.size() == y_pred.size()

        inputs_b = b_pred_masked.view(-1)
        targets_b = 1. - y_true.view(-1)

        intersection_b = (inputs_b * targets_b).sum()
        dice_coeff_b = (2. * intersection_b + smooth) / (inputs_b.sum() + targets_b.sum() + smooth)

        dice = (1. - dice_coeff + 1. - dice_coeff_b) / 2
    else:
        y_pred_n = torch.sigmoid(y_pred)
...
def bce_dice(y_true, y_pred, annotate_mask=None, weight=1.0, alpha=1.0, beta=1.0):
    bce_masked_loss = binary_crossentropy2D(y_true, y_pred, annotate_mask=annotate_mask, weight=weight)
    dice_masked_loss = dice(y_true, y_pred, annotate_mask=annotate_mask)
...
def weighted_loss2D(y_true, y_pred, annotate_mask=None, weight=1.0):
    ...
    masked_loss = bce_dice(y_true, y_pred, annotate_mask=annotate_mask, weight=weight, alpha=6.0, beta=1.0)
    return masked_loss
