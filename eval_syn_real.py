...
if __name__ == '__main__':
    ...
    if data_split == 'test':
        ...
        iou_pool = np.arange(0.0, 0.01, 0.05)
        area_pool = [5,10,20]
        savefolder = os.path.join(eval_result_folder, data_split, datasetname, opt.model_name+'_lbd'+str(int(opt.lambda_tumor))+'_run'+opt.run_number+'/ep' + epoch + opt.model_suffix)
        ...
        mask_name = 'mask'
        mask = None
        threshold_pool = np.arange(0.0, 0.11, 0.05)
        ...
        for i, data in enumerate(dataset):
            ...
            VotingMap = torch.from_numpy(visuals['pred'][0,0,:,:]).cpu().numpy()
